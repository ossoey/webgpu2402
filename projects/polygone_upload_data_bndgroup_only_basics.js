        ...
        ops.iniData = () => {
            ops.env.shaderCode = Ebk.Matrix.shaderUtils +` ` 
               ...
               @vertex fn vs(@builtin(instance_index) ii : u32, @builtin(vertex_index) vi : u32)-> VertexTransfer {
                ...
                var ld: LightDiffusion;
                ld.lightType = u32(lightparams[0]);
                ld.blendOption = u32(lightparams[1]);
                ld.lightCoords = lightcoords;
                ld.lightColor = lightcolors;
                ld.lightIntensity = lightparams[2];
                ld.lightSpectre = lightparams[3];
                ld.vxCoords = vxcoord;
                ld.vxColor = colors[vi];

    
                output.color = applyLightOnVertex(ld);
